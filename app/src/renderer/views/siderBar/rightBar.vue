  ...
  <div class="right-bar">
    <div class="topbar">
      <Row>
        ...
        <!-- <col span="4"> </col> -->
        <Col span="8">
          <el-button type="file" v-on:click="uploadClick()">
            上传
            <input
              type="file"
              v-show="0 === 1"
      ...
      <div class="main_table">
        <div>
          <Row style="padding:20px">
            <Col span="8">
              <img
                v-bind:src="'https:' + pass_msg.details.weburl + '/favicon.ico'"
        ...
        <div class="rightBarButton">
          <el-button size="small" v-on:click="changePasswordType()">
            查看密码
          </el-button>
          <el-button size="small" v-on:click="generatePassword()">
            生成随机密码
          </el-button>
          <el-button size="small" v-on:click="quitEdition()">
            放弃修改
          </el-button>
        </div>
        <!-- <div v-if="
        ...
        <div class="rightForm">
          <row class="rightForm" style="padding-left:20px; padding-right:20px">
            <input
              type="text"
          ...
          <row
            v-for="(field, index) in pass_msg.details.ps"
            :key="index"
            style="padding-left:20px; padding-right:20px"
            class="rightForm"
          >
        ...
        <div class="rightForm" style="padding-left:20px">
...
export default {
  ...
  watch: {
    ...
    DD (val) {
      ...
      this.pass_msg = JSON.parse(
        JSON.stringify(this.pass_msgs[this.$store.state.chosedPwd])
      )
    },
    ChosedPwdIndex (val) {
      debugger
  ...
  methods: {
    ...
    deletePwd () {
      ...
      var exec = require('child_process').exec
      var cmd = '0password -c delete -f ./0password.sqlite' + ' -i ' + id
      var that = this
      exec(cmd, function (error, stdout, stderr) {
        debugger
        if (error) {
    ...
    quitEdition () {
      this.pass_msg = JSON.parse(
        JSON.stringify(this.$store.state.dbData[this.$store.state.chosedPwd])
      )
    },
    uploadClick () {
